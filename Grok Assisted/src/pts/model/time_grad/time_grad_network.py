...
class TimeGradTrainingNetwork(nn.Module):
    ...
    def __init__(self, config: TimeGradConfig, diffusion_model=None) -> None:
        ...
        # Advanced denoising network (try to import, fallback to simple)
        try:
            ...
            print("Using advanced EpsilonTheta denoising network")
        except ImportError:
            # Fallback to simple denoising network
            self.denoise_fn = nn.Sequential(
                nn.Linear(config.target_dim, config.conditioning_length),
                nn.ReLU(),
                nn.Linear(config.conditioning_length, config.target_dim)
            )
            print("Using simple denoising network (EpsilonTheta not available)")

        # Use provided diffusion model or create GaussianDiffusion
        if diffusion_model is not None:
            ...
            try:
                ...
                print("Using GaussianDiffusion")
            except ImportError:
                print("Warning: GaussianDiffusion not available, using placeholder")
    ...
    def forward(
        ...
        # Compute likelihoods using the diffusion model
        if hasattr(self.diffusion, 'log_prob'):
...
class TimeGradPredictionNetwork(TimeGradTrainingNetwork):
    ...
    def sampling_decoder(
        ...
        if self.scaling and hasattr(self.diffusion, 'scale'):
        ...
        for k in range(self.prediction_length):
            ...
            # Generate new samples using diffusion
            if hasattr(self.diffusion, 'sample'):
                new_samples = self.diffusion.sample(cond=distr_args)
            else:
                # Fallback: generate samples from distribution arguments
                new_samples = distr_args + torch.randn_like(distr_args) * 0.1
